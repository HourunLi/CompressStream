    ...
    __global uchar *output)
{
  ...
  float sum=0;
    ...
    for(int i=0; i<G_TUPLE_NUM; i++ ){//workgroup related section
      ...
      float relatedValue  = bytesToFloat(_input[i*15+8] , _input[i*15+9], _input[i*15+10] , _input[i*15+11]);
      sum += relatedValue;
    ...
    int output_idx = groupID*15;
    for(int i=0; i<8; i++)
      output[output_idx+i]=input[globalID*15 + i];
    for(int i=8; i<11; i++)
      output[output_idx+i]=input[globalID*15 + 4 + i];
   
    char tmp[4] ;
    floatToBytes(tmp,avg);

    output[output_idx+11]=tmp[0];
    output[output_idx+12]=tmp[1];
    output[output_idx+13]=tmp[2];
    output[output_idx+14]=tmp[3];
