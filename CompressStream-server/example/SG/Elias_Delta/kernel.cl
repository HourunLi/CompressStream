    ...
    __global uchar *output)
{
  ...
  float sum=0;
    ...
    for(int i=0; i<G_TUPLE_NUM; i++ ){//workgroup related section
      ...
      float relatedValue  = bytesToFloat(_input[i*18+8] , _input[i*18+9], _input[i*18+10] , _input[i*18+11]);
      sum += relatedValue;
    ...
    int output_idx = groupID*18;
    for(int i=0; i<8; i++)
      output[output_idx+i]=input[globalID*18 + i];
    for(int i=8; i<14; i++)
      output[output_idx+i]=input[globalID*18 + 4 + i];
   
    char tmp[4] ;
    floatToBytes(tmp,avg);

    output[output_idx+14]=tmp[0];
    output[output_idx+15]=tmp[1];
    output[output_idx+16]=tmp[2];
    output[output_idx+17]=tmp[3];
