    ...
    __global uchar *output)
{
  ...
  float sum=0;
    ...
    for(int i=0; i<G_TUPLE_NUM; i++ ){//workgroup related section
      ...
      float relatedValue  = bytesToFloat(_input[i*24+8] , _input[i*24+9], _input[i*8+10] , _input[i*24+11]);
      sum += relatedValue;
    ...
    int output_idx = groupID*24;
    for(int i=0; i<8; i++)
      output[output_idx+i]=input[globalID*24 + i];
    for(int i=8; i<20; i++)
      output[output_idx+i]=input[globalID*24 + 4 + i];
   
    char tmp[4] ;
    floatToBytes(tmp,avg);

    output[output_idx+20]=tmp[0];
    output[output_idx+21]=tmp[1];
    output[output_idx+22]=tmp[2];
    output[output_idx+23]=tmp[3];
