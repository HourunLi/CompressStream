...
public class NSV implements Runnable {
    ...
    public static void process(String name) throws Exception {
        ...
        while ((line1 = b1.readLine()) != null) {
            ...
            if (count < TUPLE_NUM_PER_WINDOW*WINDOW_NUM_PER_BATCH) {
                ...
                col[3] = Integer.parseInt(split[3]);
                col[4] = Integer.parseInt(split[4]);
                col[5] = Integer.parseInt(split[5]);
                col[6] = Integer.parseInt(split[6]);
                startTime = System.nanoTime();
                bw.writeLong(col1);
                bw.writeFloat(col2);
                endTime = System.nanoTime();
                ...
                byte temp = 0;
                for(int i = 3; i < 7; i++) {
                    if(col[i] >=-128 && col[i] <= 127) {
                        col_num[i] = 1;
                        temp = (byte) (temp | (0 << 2*(6 - i)));
                    }
                    else if(col[i] >=-32768 && col[i] <= 32767) {
                        col_num[i] = 2;
                        temp = (byte) (temp | (1 << 2*(6 - i)));
                    }
                    else if(col[i] >=-8388608 && col[i] <= 8388607) {
                        col_num[i] = 3;
                        temp = (byte) (temp | (2 << 2*(6 - i)));
                    }
                    else if(col[i] >=-2147483648 && col[i] <= 2147483647) {
                        col_num[i] = 4;
                        temp = (byte) (temp | (0 << 2*(6 - i)));
                    }
                }
                ...
                startTime = System.nanoTime();
                bw.writeByte((byte)temp);
                for(int i = 3; i < 7; i++) {
                    if(col_num[i] == 1) {
                        bw.writeByte((byte)col[i]);
                    }
                    else if(col_num[i] == 2) {
                        bw.writeShort((short)col[i]);
                    }
                    else if(col_num[i] == 3) {
                        bw.writeByte((byte)(col[i] >> 16));
                        bw.writeShort((short)col[i]);
                    }
                    else if(col_num[i] == 4) {
                        bw.writeInt(col[i]);
    ...
    public static void main(String[] args) {
        new Thread(new NSV("1")).start();
        new Thread(new NSV("2")).start();
        // new Thread(new NSV("3")).start();
    }
