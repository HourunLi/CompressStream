...
public class DICT implements Runnable{
    ...
    public static void process(String name) throws Exception {
        ...
        while ((line1 = b1.readLine()) != null) {
            ...
            if (count == TUPLE_NUM_PER_WINDOW*WINDOW_NUM_PER_BATCH) {
                int[][] dict = new int[7][185];
                for(int i = 1; i < 7; i++){
                    if(i != 2){
                        int[] result = new int[TUPLE_NUM_PER_WINDOW*WINDOW_NUM_PER_BATCH + NumDistinct[i]];
                        startTime = System.nanoTime();
                        if(i == 1)
                            result = Col_compression.DICT(col1, NumDistinct[i]);
                        else
                            result = Col_compression.DICT(col[i], NumDistinct[i]);
                        endTime = System.nanoTime();
                        if(batchcount > START_BATCH && batchcount <= END_BATCH)
                            compress_time += endTime - startTime;
                        startTime = System.nanoTime();
                        for(int j = 0; j < TUPLE_NUM_PER_WINDOW*WINDOW_NUM_PER_BATCH; j++) {
                            if(NumDistinct[i] <= 127) {
                                bw.writeByte((byte)result[j]);
                            }
                            else if(NumDistinct[i] <= 32767) {
                                bw.writeShort((short)result[i]);
                            }
                            else if(NumDistinct[i] <= 8388607) {
                                bw.writeByte((byte)(result[i] >> 16));
                                bw.writeShort((short)result[i]);
                            }
                            else {
                                bw.writeInt(result[i]);
                            }
                        }
                        for(int j = 0; j < NumDistinct[i]; j++) {
                            dict[i][j] = result[TUPLE_NUM_PER_WINDOW*WINDOW_NUM_PER_BATCH+j];
                        }
                        endTime = System.nanoTime();
                        if(batchcount > START_BATCH && batchcount <= END_BATCH)
    ...
    public static void main(String[] args) {
        new Thread(new DICT("1")).start();
        new Thread(new DICT("2")).start();
        // new Thread(new DICT("3")).start();
    }
