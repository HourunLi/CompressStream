...
public class Elias_Gamma implements Runnable {
    ...
    public Elias_Gamma(String name) {
        this.name = name;
    }
    ...
    public static void process(String name) throws Exception {
        ...
        int[] EGDomain = Definitions.EGDomain;
        ...
        while ((line1 = b1.readLine()) != null) {
            ...
            if (count < TUPLE_NUM_PER_WINDOW*WINDOW_NUM_PER_BATCH) {
                ...
                startTime = System.nanoTime();
                s[3] = Row_compression.EncodingEliasGammaCoding(col3);
                s[4] = Row_compression.EncodingEliasGammaCoding(col4);
                s[5] = Row_compression.EncodingEliasGammaCoding(col5);
                s[6] = Row_compression.EncodingEliasGammaCoding(col6);
                endTime = System.nanoTime();
                if(batchcount > START_BATCH && batchcount <= END_BATCH)
    ...
    public static void main(String[] args) {
        new Thread(new Elias_Gamma("1")).start();
        new Thread(new Elias_Gamma("2")).start();
        // new Thread(new Elias_Gamma("3")).start();
    }
